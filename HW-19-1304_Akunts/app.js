...
users.forEach((user) => {
  ...
  div.innerText = user.name + " / " + user.username;
  document.querySelector("#left").appendChild(div);
});

const userList = document.querySelectorAll("#left > div");
...
for (user of userList) {
  user.addEventListener("click", (event) => {
    ...
    found.forEach((user) => {
      ...
      h3.innerText = user.name + " - " + user.username;
      const h4 = document.createElement("h4");
      h4.innerText = user.email;
      div.appendChild(h3);
      div.appendChild(h4);
      div.appendChild(getAlbumInfo(user.id));
      document.querySelector("#right").appendChild(div);
    });
  });
}

const getAlbumInfo = (userId) => {
  const div = document.createElement("div");
  //   const h3 = document.createElement("h3");
  //   h3.innerText = "Albums";
  ...
  filteredAlbums.forEach((album) => {
    ...
    ul.appendChild(li);
    div.appendChild(ul);
    div.appendChild(getPics(album.id));
  });
  return div;
};

const getPics = (albumId) => {
  const div = document.createElement("div");
  div.classList.add("flex");
  const filteredPics = photos.filter((photo) => photo.albumId === albumId);

  filteredPics.forEach((photo) => {
...
// const albumDescr = document.querySelectorAll("ul >li");
// console.log(albumDescr);

// for (item of albumDescr) {
//   item.addEventListener("click", (event) => {
//     console.log(event.currentTarget);
//     const id = +event.currentTarget.id.split("_")[1];
//     const found = albums.filter((album) => album.id === id);
//     found.forEach((album) => {
//       section.appendChild(getImages(album.id));
//       document.querySelector("#pics").appendChild(div);
//     });
//   });
// }
